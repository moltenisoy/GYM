...
def demo_class_schedules():
    ...
    try:
        response = requests.get(f"{BASE_URL}/clases/horarios")
        data = response.json()
...
def demo_book_class():
    ...
    try:
        ...
        # Ver mis reservas
        response = requests.get(f"{BASE_URL}/clases/mis-reservas?username={USERNAME}")
        data = response.json()
...
def demo_equipment():
    ...
    try:
        response = requests.get(f"{BASE_URL}/equipos")
        data = response.json()
...
def demo_exercises():
    ...
    try:
        response = requests.get(f"{BASE_URL}/ejercicios")
        data = response.json()
...
def demo_api_status():
    ...
    try:
        ...
        print(f"   Ejecuta: python -m uvicorn madre_server:app --host 0.0.0.0 --port 8000")
        return False
